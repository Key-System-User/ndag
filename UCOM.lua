...
local function removeCOM(model)
...
local function setupCharacter(char)
    ...
    if charConnections[char] then
        for _, c in pairs(charConnections[char]) do c:Disconnect() end
    end
    charConnections[char] = {}

    table.insert(charConnections[char], char.ChildAdded:Connect(function(c)
        if c.Name ~= "COM_BALANCER" and enabled then
            task.wait()
            setCOM(char, VISIBLE, BRUTE_FORCE)
        end
    end))

    table.insert(charConnections[char], char.ChildRemoved:Connect(function(c)
        if c.Name ~= "COM_BALANCER" and enabled then
            task.wait()
            setCOM(char, VISIBLE, BRUTE_FORCE)
        end
